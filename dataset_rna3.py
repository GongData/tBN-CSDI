...
DATA_PATH = "data/mESC/ExpressionData.csv"
CACHE_DIR = "data/mESC/cache"
...
def parse_new_rna_data(df, missing_ratio=0.1):
    ...
    # Inject additional missingness randomly over observed (nonzero) entries
...
def get_new_rna_dataloader(file_path=DATA_PATH, seed=1, batch_size=16, missing_ratio=0.1):
    ...
    # Shuffle indices randomly using numpy's random generator
    rng = np.random.default_rng(seed)
    all_indices = np.arange(num_cells)
    rng.shuffle(all_indices)

    # Define split sizes (e.g., 60% train, 20% valid, 20% test)
